export const gradients = {
  primary: "bg-gradient-to-r from-[#E7D9FF] to-[#FADCEB]",
  soft: "bg-gradient-to-r from-[#F8F3FF] to-[#FFF7FB]",
  pink: "bg-gradient-to-r from-[#F8DFFF] to-[#FFE7D1]",
  brand: "bg-[linear-gradient(135deg,#E7D9FF,#F8DFFF,#FADCEB,#FFE7D1)]",
};

export const backgroundSoft = "#FFF7FB"; // pink whisper
export const backgroundAlt = "#F8F3FF"; // lilac mist

export const glass =
  "bg-white/80 backdrop-blur-xl border border-white/60 shadow-[0_var(--space-4)_30px_rgba(180,150,200,0.10)] rounded-3xl";

export const cardSoft = "bg-white rounded-lg shadow-[0_3px_var(--space-5)_rgba(120,110,200,0.42)]";

export const sectionSpacing = "space-y-5";

export const fontSoft = "font-sans tracking-tight";
